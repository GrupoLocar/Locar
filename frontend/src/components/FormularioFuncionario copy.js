...
const limites = {
  ...
  estado: 2, // <= NOVO: UF com 2 caracteres
...
const FormularioFuncionario = forwardRef(({ funcionario = {}, onSalvar, onNovo }, ref) => {
  ...
  const [dados, setDados] = useState({
    ...
    estado: '' // <= NOVO: garante chave no estado
  });
  ...
  useEffect(() => {
    ...
    setDados((prev) => {
      ...
      return { ...prev, ...funcionarioNormalizado };
    });
  }, [funcionario]);
  ...
  const salvar = async () => {
    const ignorarCampos = ['observacao', 'cnh_arquivo', 'comprovante_residencia', 'nada_consta', 'comprovante_mei', 'curriculo', 'pj'];
  ...
  // === NOVO: força a ordem com 'estado' logo após 'municipio'
  const renderCampos = (() => {
    ...
    if (idxMunicipio !== -1) {
      ...
      // se 'municipio' não estiver na lista por algum motivo, garante que 'estado' exista
      arr.push('estado');
    }
    ...
    if (idxAdmissao !== -1) {
      ...
      // fallback: se 'data_admissao' não estiver na lista original, ainda assim exibe
